...
# ---------------------------------------------------------------------------
# Personal Documents (User Library)
# ---------------------------------------------------------------------------


@router.get("/documents/personal", response_model=list[DocumentRead])
async def list_personal_documents(
    context: UserContext = Depends(get_current_user_context),
    db: AsyncSession = Depends(get_db),
):
    return await service.list_personal_documents(db, context.id)


@router.post("/documents/personal", response_model=DocumentUploadResponse)
async def upload_personal_document(
    ...
    document = await service.upload_personal_document(
        db,
        filename=file.filename or "untitled",
        content_type=file.content_type or "application/octet-stream",
        file_bytes=file_bytes,
...
@router.delete("/documents/personal/{document_id}", status_code=204)
async def delete_personal_document(
    document_id: uuid.UUID,
    context: UserContext = Depends(get_current_user_context),
    db: AsyncSession = Depends(get_db),
):
    await service.delete_personal_document(db, document_id, context.id)
