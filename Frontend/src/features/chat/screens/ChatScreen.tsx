...
export function ChatScreen() {
  const { conversation, conversationId, messages, isStreaming, isLoadingConversation, error, sendMessage, loadConversation, startNewConversation, linkProject } = useChat();
  ...
  return <main className="flex h-full min-h-0 flex-col overflow-hidden bg-white">
      ...
      <div className="mx-auto flex max-w-3xl flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          {conversation?.type !== "project_group" && <>
            ...
            <select id="linked-project" value={conversationId ? conversation?.linked_project_id ?? "" : newLinkedProjectId} onChange={(event) => conversationId ? void changeLinkedProject(event.target.value || null) : setNewLinkedProjectId(event.target.value)} className="rounded-lg border border-gray-200 bg-white p-1.5 text-xs"><option value="">بدون پروژه</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select>
          </>}
    ...
    <div className="min-h-0 flex-1"><ChatWindow messages={messages} isStreaming={isStreaming} isLoadingConversation={isLoadingConversation} error={error} onSend={(text) => void sendMessage(text, newLinkedProjectId || null)} group={conversation?.type === "project_group"} senderNames={senderNames} onUpload={conversationId ? async (file) => { await chatApi.uploadDocument(conversationId, file); } : undefined} /></div>
